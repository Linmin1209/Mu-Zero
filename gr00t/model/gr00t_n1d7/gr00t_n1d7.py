...
class Gr00tN1d7(PreTrainedModel):
    ...
    def __init__(
        ...
        if getattr(config, "use_adaptive_component_head", False):
            ...
            self.action_head = build_component_factored_action_head(Gr00tN1d7ActionHead)(config)
            logger.info(
                "Using ComponentFactoredActionHead (native AlternateVLDiT + per-component decoders)"
            )
        else:
